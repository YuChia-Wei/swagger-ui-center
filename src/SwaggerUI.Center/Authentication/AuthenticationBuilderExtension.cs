...
using SwaggerUI.Center.Authentication.OpenIdConnect;
using SwaggerUI.Center.Authentication.Options;

namespace SwaggerUI.Center.Authentication;
...
public static class AuthenticationBuilderExtension
{
    ...
    public static void AddSwaggerAuthentication(this IServiceCollection serviceCollection, ConfigurationManager configurationManager)
    {
        ...
        ArgumentNullException.ThrowIfNull(swaggerAuthSettingOptions);

        var authenticationBuilder = swaggerAuthSettingOptions.Default switch
        {
        ...
        if (swaggerAuthSettingOptions.Opid?.IsSettled ?? false)
        {
            authenticationBuilder.AddOpenIdConnectWithCookie(swaggerAuthSettingOptions.Opid);
        }

        if (swaggerAuthSettingOptions.Jwt?.IsSettled ?? false)
        {
            authenticationBuilder.AddJwtAuthentication(swaggerAuthSettingOptions.Jwt);
        }
    }
